...
-(void)setGETRequestWithUrlStr:(NSString *)urlStr
{
    ...
    [manager GET:[urlStr stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] parameters:nil progress:nil success:^(NSURLSessionDataTask *task, id responseObject)
     {
         [self requestSucessWithOperation:task andObject:responseObject];
     }
...
{
    RequestManager *manager = [RequestManager manager];
    [manager POST:[urlStr stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] parameters:paramStr progress:nil success:^(NSURLSessionDataTask *task, id responseObject)
     {
         [self requestSucessWithOperation:task andObject:responseObject];
     }
         failure:^(NSURLSessionDataTask *task, NSError *error)
...
-(void)requestSucessWithOperation:(NSURLSessionDataTask *)operation andObject:(id)responseObject
{
